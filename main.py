...
현재는 안 쓰지만, num_workers를 바꿔서 코어를 여러개 동시에 활용하는 기법 시도 중 문제 발생
Windows는 spawn() 방식을 써서 코드를 처음부터 다시 실행해 환경을 하나씩 구축하는 방식
main 안에서 워커를 만드는데, 그 main이 워커에 의해 다시 실행되면 무한 반복
따라서 if __name__ '__main__'으로 파일을 직접 실행했을 때만 코드가 돌아가도록 해야 함

참조 API / torch.utils.data - 파이토치 공식 문서
https://docs.pytorch.org/docs/2.12/data.html
Wrap most of you main script’s code within if __name__ == '__main__': block, 
to make sure it doesn’t run again (most likely generating error) when each worker process is launched. 
You can place your dataset and DataLoader instance creation logic here, as it doesn’t need to be re-executed in workers.
"""
}
if __name__ == "__main__":
    ...
    for cur_model_name in settings.MODEL_NAME_LIST:
        ...
        if settings.CAN_USE_DIFFERENTIAL_LEARNING_RATE:
            # 마지막 레이어 이름만 가져오기
            last_layer_name = settings.MODEL_INFO_DICT[cur_model_name][2]
        ...
        if is_my_model_mode:
            ...
            if best_val_acc > 0.0: # 최고 모델의 최고 정확도가 0보다 크면
                ...
                with open(setting_path, "w", encoding="utf-8") as file: # w(쓰기 모드), 인코딩은 한글 안 깨지는 utf-8
                    ...
                    augmentation_mode_name = augmentation_maker.get_augmentation_mode_name(settings.AUGMENTATION_MODE_NAME)
                    file.write(f"데이터 증대: {f'적용({augmentation_mode_name})' if settings.CAN_USE_AUGMENTATION else '미적용'}\n")
