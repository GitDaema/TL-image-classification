...
class DataManager:
    ...
    def __init__(self, setting):
        ...
        data = datasets.ImageFolder(root=self.setting.TRAIN_DIR)
    ...
    def get_loaders(self, train_transform, val_transform):
        ...
        train_data_set = datasets.ImageFolder(root=self.setting.TRAIN_DIR, transform=train_transform)
        val_data_set = datasets.ImageFolder(root=self.setting.VAL_DIR, transform=val_transform)
        
        # DataLoader를 이용해 데이터셋을 쪼개서 배치 단위로 묶어 모델 학습 및 평가용 객체로 저장
        # 배치는 한 번 학습할 때 동시에 처리하는 데이터의 묶음, 이번 프로젝트에서는 이미지 개수
        # 학습 내는 섞어야 편향을 막을 수 있음
        train_loader = DataLoader(train_data_set, batch_size=self.setting.BATCH_SIZE, shuffle=True)
        val_loader = DataLoader(val_data_set, batch_size=self.setting.BATCH_SIZE, shuffle=False)
